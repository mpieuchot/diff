 ...
 * in memory.
 *
 * TODO: the divide and conquer requires linear *space*, not necessarily linear
 * *time*.  It recurses, apparently doing multiple Myers passes, and also it
 * apparently favors fragmented diffs in cases where chunks of text were moved
 * to a different place. Up to a given count of diff atoms (text lines), it
 * might be desirable to accept the quadratic memory usage, get nicer diffs and
 * less re-iteration of the same data?
 */
 ...
 * Diagonals indicate identical atoms on both sides, the challenge is to use
 * as many diagonals as possible.
 *
 ...
 *
 * Only one forward column and one backward column are kept in mem, each need
 * at most left.len + 1 + right.len items.
 * Note that each d step occupies either the even or the odd items of a column:
 * if e.g. the previous column is in the odd items, the next column is formed
 * in the even items, without overwriting the previous column's results.
 *
 ...
 *		function. This is carried over between invocations with
 *		increasing d. kd_forward points at the center of the state
 *		array, allowing negative indexes.
 *
 * kd_backward: the traversal state for backwards traversal, to find a meeting
 *		point. Since forwards is done first, kd_backward will be valid
 *		for d - 1, not d. kd_backward points at the center of the state
 *		array, allowing negative indexes.
 *
 * d:		Step or distance counter, indicating for what value of d the
 *		kd_forward should be populated.  For d == 0, kd_forward[0] is
 *		initialized, i.e. the first invocation should be for d == 0.
 *
	...
	for (k = d; k >= -d; k -= 2) {
		if (k < -(int)right->atoms.len || k > (int)left->atoms.len) {
			...
			if (k < 0) {
				 ...
				 * below the entire graph, nothing will come
				 * of this.
				 */
				debug(" break");
				break;
		...
		if (d == 0) {
			 ...
			 * yet, get the initial x from the top left of the
			 * Myers graph.
			 */
			x = 0;
		}
		 ...
		 */
			...
			/*
			 * Advance from k - 1.
			 * From position prev_k, step to the right in the
			 * Myers graph: x += 1.
			 */
			int prev_k = k - 1;
			...
			/* The bottom most one.
			 * From position prev_k, step to the bottom in the
			 * Myers graph: y += 1.
			 *
		 ...
		 *
		 * If the delta in length is odd, then d and backwards_d hit
		 ...
		 *      |
		 *
		 * So in the forward path, we can only match up diagonals
		 * when the delta is odd.
		 */
		/*
 ...
 * right:	the right side to diff against.
 *
 * kd_forward:	the traversal state for forwards traversal, to find a
 *		meeting point.  Since forwards is done first, after this,
 *		both kd_forward and kd_backward will be valid for d.
 *		kd_forward points at the center of the state array, allowing
 *		negative indexes.
 *
 * kd_backward: the traversal state for backwards traversal, to find a meeting
 *		point.  This is carried over between invocations with
 *		increasing d.  kd_backward points at the center of the state
 *		array, allowing negative indexes.
 *
 * d:		Step or distance counter, indicating for what value of d the
 *		kd_backward should be populated.  Before the first invocation,
 *		kd_backward[0] shall point at the bottom right of the Myers
 *		graph (left.len, right.len).  The first invocation will be for
 *		d == 1.
 *
 * meeting_snake: resulting meeting point, if any.
 */
    ...
    int *kd_forward, int *kd_backward, int d, struct diff_box *meeting_snake)
{
	...
	for (c = d; c >= -d; c -= 2) {
		...
		/*
		 * Favoring "-" lines first means favoring moving rightwards
		 * in the Myers graph.  For this, all c should derive from
		 * c - 1, only the bottom most c derive from c + 1:
		 *
		 *                                  2   1   0
		 *  ---------------------------------------------------
		 *                                               c=
		 ...
		 * If c < d, use c + 1 in case that yields a larger x. Also use
    ...
    struct diff_state *state)
{
	 ...
	 * point the kd to the middle.  It is then possible to index from
	 * -max/2 .. max/2.
	 */
	kd_forward += max/2;
	kd_backward += max/2;
	...
	for (d = 0; d <= (max/2); d++) {
		...
		if (!diff_box_empty(&mid_snake))
			break;
		diff_divide_myers_backward(left, right, kd_forward, kd_backward,
		    d, &mid_snake);
		if (!diff_box_empty(&mid_snake))
			break;
	}
	...
	if (diff_box_empty(&mid_snake)) {
		 ...
		 * i.e. nothing before the mid-snake.
		 */
		...
		/*
		 * else: left_section_len == 0 and right_section_len == 0,
		 * i.e. nothing after the mid-snake.
		 */
	}
    ...
    struct diff_state *state)
{
	...
	if (kd_buf_size < kd_len /* overflow? */ ||
		...
		debug("state size %zu > permitted_state_size %zu,"
		    " use fallback_algo\n", kd_buf_size,
		    algo_config->permitted_state_size);
	...
	/*
	 * The 'k' axis in Myers spans positive and negative indexes,
	...
	for (d = 0; d <= max; d++, kd_column += kd_len) {
		...
		for (k = d; k >= -d; k -= 2) {
			...
			if (d == 0) {
				 ...
				 * graph.
				 * If k == d, there is no k + 1 and k - 1 is
				 * the only option.
				...
				if (k > -d && (k == d ||
					 ...
					 * decrementing k while keeping the
					 * same x. (since we're deriving y from
					 * y = x - k).
					 */
					int prev_k = k + 1;
					int prev_x = kd_prev_column[prev_k];
	 ...
	 *      |
	 *
	 * From (4,4) backwards, find the previous position that is the
	 * largest, and remember it.
	 *
	 */
	for (d = backtrack_d, k = backtrack_k; d >= 0; d--) {
		 ...
		 * kd_column.  That kd_column is no longer needed otherwise,
		 * so just re-purpose kd_column[0] = x and kd_column[1] = y,
		...
		debug("prev[k-1] = %d,%d  prev[k+1] = %d,%d\n",
		    kd_prev_column[k - 1],
		    xk_to_y(kd_prev_column[k - 1], k - 1),
		    kd_prev_column[k + 1],
		    xk_to_y(kd_prev_column[k + 1], k + 1));
		if (y == 0 ||
			...
			debug("prev k=k-1=%d x=%d y=%d\n", k,
			    kd_prev_column[k], xk_to_y(kd_prev_column[k], k));
		} else {
			k = k + 1;
			debug("prev k=k+1=%d x=%d y=%d\n", k,
			    kd_prev_column[k], xk_to_y(kd_prev_column[k], k));
	...
	for (d = 0; d <= backtrack_d; d++, kd_column += kd_len) {
		...
		if (left_section_len && right_section_len) {
			 ...
			 * (except when starting at (0,0)). Find out whether
			 * the lead-in is horizontal or vertical:
			 *
			 *     left
			 *  ---------->
			 *  |
			 ...
			 * is horizontal, meaning first remove one atom from
			 * the left before sliding down the snake.
			...
			if (left_section_len == right_section_len + 1) {
				...
				/*
				 * The numbers are making no sense. Should
				 * never happen.
				 */
				rc = DIFF_RC_USE_DIFF_ALGO_FALLBACK;
				goto return_rc;
